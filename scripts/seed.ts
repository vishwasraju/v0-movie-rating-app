...
// Extract unique users from reviews for profiles
const uniqueUsers = Array.from(new Set(reviews.map(r => r.userId))).map(userId => {
...
async function seed() {
    ...
    const { error: moviesError } = await supabase.from('movies').upsert(
        movies.map(m => ({
            id: m.id,
            title: m.title,
            year: m.year,
            ...
            backdrop: m.backdrop,
            rating: m.rating,
            rating_count: m.ratingCount, // map camelCase to snake_case
            runtime: m.runtime,
            genres: m.genres,
            director: m.director,
            cast: m.cast,
            plot: m.plot,
            release_date: m.releaseDate // map camelCase to snake_case
        }))
    )
    ...
    // 2. Insert Profiles
    console.log(`Inserting ${uniqueUsers.length} profiles...`)
    const { error: profilesError } = await supabase.from('profiles').upsert(uniqueUsers)
    if (profilesError) console.error('Error inserting profiles:', profilesError)
    ...
    const { error: reviewsError } = await supabase.from('reviews').upsert(
        reviews.map(r => ({
            id: r.id,
            movie_id: r.movieId, // map camelCase
            user_id: r.userId, // map camelCase
            user_name: r.userName, // map camelCase
            rating: r.rating,
            title: r.title,
            content: r.content,
            helpful: r.helpful,
            created_at: r.createdAt // map camelCase
        }))
    )
